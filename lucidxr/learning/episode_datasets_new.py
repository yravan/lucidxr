...
class EpisodeDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if self.prune_cache:
            cprint("[EpisodeDataset.__init__] prune_cache=True, clearing cache_root contents", "red")
                ...
                try:
                    ...
                    for child in Path(cache_root).iterdir():
                        if child.is_file() or child.is_symlink():
                            print(f"[EpisodeDataset.__init__] unlink file/symlink: {child}", flush=True)
                            child.unlink()
                        elif child.is_dir():
                            print(f"[EpisodeDataset.__init__] rmtree dir: {child}", flush=True)
                            prune_safe(child)
                except Exception as e:
                    print(f"[EpisodeDataset.__init__] Failed to clear cache: {e}", flush=True)
                    raise e
