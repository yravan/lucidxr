...
class ObjMujocoObject(Body):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ------------------------------------------------------------------ #
        visual_mesh_lines = "\n".join(
            f'    <mesh name="{name}_vis_{i}" file="{assets}/{name}_{i}.obj" scale="{scale} {scale} {scale}"/>'
        ...
        for i, tex in enumerate(textures):
            if tex:
                tex_and_mat_lines.append(
                    f'    <texture type="2d" name="{name}_tex_{i}" file="{assets}/{tex}.png"/>'
                )
                tex_and_mat_lines.append(
                    f'    <material name="{name}_mat_{i}" texture="{name}_tex_{i}" specular="0.5" shininess="0.25"/>'
                )
...
{collision_mesh_lines}
        ...
        for i in range(visual_count):
            if textures[i]:
                visual_geom_lines.append(
                    f'    <geom mesh="{name}_vis_{i}" material="{name}_mat_{i}" class="{name}_visual"/>'
                )
            else:  # no material
        ...
        for i in range(collision_count):
            rgba = f"{rng.random()} {rng.random()} {rng.random()} 1" if randomize_colors else "0.8 0.8 0.8 1"
            collision_geom_lines.append(
