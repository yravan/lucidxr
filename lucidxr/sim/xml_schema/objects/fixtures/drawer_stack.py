...
from lucidxr.sim.xml_schema.objects.fixtures.drawer import KitchenDrawer
from lucidxr.sim.xml_schema.objects.fixtures.drawer_visual import KitchenDrawerVisual
from lucidxr.sim.xml_schema.objects.fixtures.single_cabinet import KitchenSingleCabinet
...
class DrawerStack(Body):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if visual:
            drawer = KitchenDrawerVisual(
                assets=f"{assets}/drawer",
                width=width,
                attributes=dict(name=f"{name}_drawer", pos=f"0 0 {self._Z_DRAWER}", quat="1 0 0 0"),
                **kwargs,
            )
        else:
        ...
        cabinet = KitchenSingleCabinet(
            assets=f"{assets}/cabinet",
            attributes=dict(name=f"{name}_cab", pos=f"0 0 {self._Z_CABINET}", quat="1 0 0 0"),
            **kwargs,
        )
