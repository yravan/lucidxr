...
class ObjaverseMujocoSpoon(Body):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(collision_count):
            rgba = (
                f"{rng.random()} {rng.random()} {rng.random()} 0.3" if randomize_colors else "0.8 0.8 0.8 1"
            )
            collision_geoms += f'<geom mesh="{name}_coll_{i}" rgba="{rgba}" class="{name}_collision"/>'
