"""Rotation representations; optional tensor outputs are detached, not differentiable."""
...
def gs62quat(sixd):
    ...
    is_torch = torch is not None and isinstance(sixd, torch.Tensor)
    if not is_torch:
...
def mat2quat(rot_flat):
    ...
    is_torch = torch is not None and isinstance(rot_flat, torch.Tensor)
    ...
    quats = quats.reshape(original_shape[:-1] + (4,))
