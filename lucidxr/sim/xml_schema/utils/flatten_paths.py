...
def get_hash_key(file_path: str, hash_length: int = 8) -> str:
    ...
    return (
        f"{hash_obj.hexdigest()[:hash_length]}.{file_ext}" if file_ext else hash_obj.hexdigest()[:hash_length]
    )


def flatten_paths(
    file_paths: list[str], prefix: str = None, levels_to_preserve: int = 0
    ...
    if levels_to_preserve < 0:
        raise ValueError("levels_to_preserve must be nonnegative")
    hash_lookup_table = {}
    path_dict = {}
    ...
    for path in file_paths:
        ...
        if key in path_dict and os.path.normpath(path_dict[key]) != os.path.normpath(path):
            raise ValueError(f"Asset path collision: {path_dict[key]!r} and {path!r} map to {key!r}")
        path_dict[key] = path
