...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.randomize_wrist = False

    TrainArgs.load_checkpoint = None
    TrainArgs.num_steps = 50_000
    ...
    ACT_Config.image_keys = ["wrist/lucid", "left/lucid", "right/lucid"]
    with sweep.product:
...
def tail(RUN, TrainArgs, ACT_Config):
    camera_str = "cam-aug" if TrainArgs.aug_camera_randomization else "cam-fixed"
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__,
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunksize-{ACT_Config.chunk_size}/{camera_str}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
