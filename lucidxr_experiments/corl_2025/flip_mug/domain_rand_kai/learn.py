...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.product:
            ACT_Config.image_keys = [("back/domain_rand", "wrist/domain_rand", "right/domain_rand")]
...
def tail(RUN, TrainArgs, ACT_Config):
    cam_id = 0
    if ACT_Config.image_keys == ("back/domain_rand", "wrist/domain_rand", "right/domain_rand"):
        cam_id = 1
