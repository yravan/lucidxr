...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
      ACT_Config.kl_weight = [0.1, 1]
      ACT_Config.lr = [1e-5, 5e-5, 1e-4]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"mug_tree_kl_lr_sweep/{time:%Y/%m/%d/%H-%M-%S}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}/chunk-size-{ACT_Config.chunk_size}",
    )
