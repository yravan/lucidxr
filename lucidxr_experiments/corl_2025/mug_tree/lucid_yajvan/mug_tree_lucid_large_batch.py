...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.zip:
        ...
        ACT_Config.lr_backbone = [1e-5, 5e-5]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"mug_tree_lucid_large_batch/{time:%Y/%m/%d/%H-%M-%S}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
