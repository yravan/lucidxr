...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.image_keys = [["right/rgb", "wrist/rgb", "right_r/rgb"], ["right/rgb", "wrist/rgb", "top/rgb"]]
    ...
    with sweep.zip:
        ...
        ACT_Config.lr_backbone = [2.5e-5, 5e-5]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"mug_tree_three_view_large_batch/{time:%Y/%m/%d/%H-%M-%S}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
