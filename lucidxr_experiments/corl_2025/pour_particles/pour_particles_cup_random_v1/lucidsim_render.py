...
master_config = dict(
    name="particle_pour",
    env_name="ParticlePour-cup_rand-lucid-v1",
    camera_keys=[
        "wrist/lucid",
        "right/lucid",
        "left/lucid",
...
demo_prefix = "/lucidxr/lucidxr/datasets/lucidxr/corl-2025/particle_pour/2025/07/02/22.03.43"
demo_ep_start = 1
demo_ep_end = 63

num_render_workers = 65
render_worker_chain_length = 6

num_generative_workers = 42
...
# for generative_queue_name in generative_queue_names[1:2]:
...
for i in range(0, num_generative_workers, generative_worker_chain_length):
    ...
    job = jaynes.add(
        ...
        dry_run=False,
        overwrite=overwrite,
        demo_prefix=demo_prefix.format(**get_config(i * generative_worker_chain_length)),
    ...
    for _ in range(1, generative_worker_chain_length):
        if i + _ <= num_generative_workers:
            job = job.chain(
                ...
                dry_run=False,
                overwrite=overwrite,
                demo_prefix=demo_prefix.format(**get_config(i * generative_worker_chain_length + _)),
