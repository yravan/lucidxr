...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.dataset_prefix = [
        "lucidxr/lucidxr/datasets/lucidxr/corl-2025/robosuite_stack/2025/06/18/18.16.48/"
    ]
    ...
    with sweep.product:
        ...
        with sweep.product:
            ACT_Config.image_keys = [("right/rgb", "wrist/rgb", "front/rgb", "right_r/rgb")]
...
def tail(RUN, TrainArgs, ACT_Config):
    cam_id = 0
    if ACT_Config.image_keys == ("right/rgb", "wrist/rgb", "front/rgb", "right_r/rgb"):
        cam_id = 1
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__,
        job_name=f"corl_2025/pick_place_v1/{time:%Y/%m/%d/%H-%M-%S}/chunksize-{ACT_Config.chunk_size}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{cam_id}/{TrainArgs.seed}",
    )
