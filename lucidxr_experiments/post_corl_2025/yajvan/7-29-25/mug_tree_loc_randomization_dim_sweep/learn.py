...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        TrainArgs.batch_size_train = [56]
        TrainArgs.batch_size_eval = [56]
        ACT_Config.kl_weight = [10, 1e-4]
        with sweep.zip:
            ACT_Config.dim_feedforward = [3200, 3200, 256]
            ACT_Config.head_dim = [512, 128, 512]
        TrainArgs.seed = [0]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/kl-{ACT_Config.kl_weight}/dim_feedforward-{ACT_Config.dim_feedforward}/head_dim-{ACT_Config.head_dim}",
    )
...
with Sweep(RUN, Unroll, ACT_Config) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            ...
            Unroll.render = [True for _ in range(10)] + [False for _ in range(90)]  # only render 10% of the runs
