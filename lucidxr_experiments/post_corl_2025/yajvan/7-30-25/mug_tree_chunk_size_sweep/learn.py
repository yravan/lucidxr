...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.chunk_size = [25, 75, 100]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunksize-{ACT_Config.chunk_size}/kl-{ACT_Config.kl_weight}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb','')}",
    )
