...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.lr = 1e-5
    ACT_Config.lr_backbone = 1e-6
    TrainArgs.batch_size_train = 8
    TrainArgs.batch_size_eval = 8
    TrainArgs.dataset_prefix = demo_prefixes[2:4]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb','')}",
    )
