...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.dataset_prefix = ["/lucidxr/lucidxr/datasets/lucidxr/corl-2025/pick_place/2025/06/21/16.04.23/"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunk_size-{ACT_Config.chunk_size}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb', '')}"
    )
