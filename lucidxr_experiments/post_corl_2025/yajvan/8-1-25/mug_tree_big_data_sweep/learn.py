...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        TrainArgs.dataset_prefix = [demo_prefixes[:i] for i in range(1, len(demo_prefixes) + 1)]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/high-kl/num_demos-{len(TrainArgs.dataset_prefix) * 50}"
    )
