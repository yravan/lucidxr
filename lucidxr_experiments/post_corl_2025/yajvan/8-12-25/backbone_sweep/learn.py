...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.eval_interval = 4000
    TrainArgs.checkpoint_interval = 4000
    ...
    with sweep.product:
        ACT_Config.enc_layers = [2,4]
        ACT_Config.backbone = ["resnet18", "resnet18_untrained", "simple"]
        with sweep.zip:
            ACT_Config.chunk_size = [25, 25]
            TrainArgs.num_steps = [30000, 90000]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('rgb','')}/backbone-{ACT_Config.backbone}/enc_layers-{ACT_Config.enc_layers}"
    )
