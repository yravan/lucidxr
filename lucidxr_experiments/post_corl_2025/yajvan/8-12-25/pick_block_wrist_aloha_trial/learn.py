...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.batch_size_val = 8
    TrainArgs.batch_size_train = 8
    ACT_Config.lr = 5e-5
    ACT_Config.lr_backbone = 5e-6
    ACT_Config.detr_intermediate_layer = 0
    ACT_Config.dim_feedforward = 3200
    ACT_Config.head_dim = 512
    ACT_Config.clip_max_norm = 0.0
    with sweep.product:
        ACT_Config.chunk_size = [50]
        TrainArgs.num_steps = [100000]
        with sweep.zip:
            ACT_Config.image_keys = [["wrist/domain_rand"], ["right/domain_rand", "wrist/domain_rand", "left/domain_rand"]]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('domain_rand','')}"
    )
...
with Sweep(RUN, Unroll, ACT_Config) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            ...
            Unroll.image_keys = [["right/rgb", "wrist/rgb", "left/rgb"], ["right/domain_rand", "wrist/domain_rand", "left/domain_rand"]]
        with sweep.zip:
