...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            ...
            ACT_Config.image_keys = [["wrist/depth"]]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/backbone-{ACT_Config.backbone}/wrist"
    )
...
with Sweep(RUN, Unroll, ACT_Config) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = ["PickPlaceRobotRoom-single_random-depth-v1"]
            Unroll.image_keys = [["right/depth", "wrist/depth", "left/depth"]]
        with sweep.zip:
