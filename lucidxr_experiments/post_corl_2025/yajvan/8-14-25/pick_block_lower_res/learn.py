...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.image_keys = [["right/lucid", "wrist/lucid", "left/lucid"], ["right/domain_rand", "wrist/domain_rand", "left/domain_rand"], ["right/rgb", "wrist/rgb", "left/rgb"]]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{ACT_Config.image_keys[0].split('/')[-1]}"
    )
