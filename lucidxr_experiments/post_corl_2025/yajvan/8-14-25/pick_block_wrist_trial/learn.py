...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.image_keys = [["wrist/rgb"], ["wrist/domain_rand"]]
        TrainArgs.num_steps = [60000]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{ACT_Config.image_keys[0].split('/')[-1]}/backbone-{ACT_Config.backbone}/dec_layers-{ACT_Config.dec_layers}/downsample-{ACT_Config.downsample_images}"
    )
