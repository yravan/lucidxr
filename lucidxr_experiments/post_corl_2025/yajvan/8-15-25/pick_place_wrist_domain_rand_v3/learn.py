...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.delta_space = True
    with sweep.product:
        ACT_Config.downsample_images = [1]
        ACT_Config.enc_layers = [4]
        ACT_Config.dec_layers = [1]
        with sweep.zip:
            ACT_Config.head_dim = [128, 512]
            ACT_Config.dim_feedforward = [256,3200]
        ACT_Config.nheads = [8, 16]
        ACT_Config.lr_backbone = [1e-4, 2e-4]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/headdim-{ACT_Config.head_dim}/nheads-{ACT_Config.nheads}/lr_backbone-{ACT_Config.lr_backbone}"
    )
