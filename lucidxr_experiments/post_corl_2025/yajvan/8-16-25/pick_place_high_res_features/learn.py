...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'low-res' if ACT_Config.downsample_images > 1 else 'high-res'}/enc-{ACT_Config.enc_layers}/dec-{ACT_Config.dec_layers}/nheads-{ACT_Config.nheads}/headdim-{ACT_Config.head_dim}",
    )
