...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.lr_backbone = [3e-4,4e-4,5e-4]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/low-res/dec-{ACT_Config.dec_layers}/nheads-{ACT_Config.nheads}/lr_backbone-{ACT_Config.lr_backbone}"
    )
