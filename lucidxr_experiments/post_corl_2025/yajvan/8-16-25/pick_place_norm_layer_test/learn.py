...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.dec_layers = [2,4,7]
        ACT_Config.nheads = [8, 16]
        ACT_Config.lr_backbone = [2e-4]
        ACT_Config.resnet_layer = [4]
        ACT_Config.norm_layer = ["BatchNorm2d", "LayerNorm", "GroupNorm"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'low-res' if ACT_Config.downsample_images > 1 else 'high-res'}/norm-{ACT_Config.norm_layer}/dec_layers-{ACT_Config.dec_layers}/nheads-{ACT_Config.nheads}"
    )
