...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.nheads = [16]
        ACT_Config.lr_backbone = [2e-4]
        ACT_Config.backbone = ["resnet18_untrained"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/enc-{ACT_Config.enc_layers}/dec-{ACT_Config.dec_layers}/nheads-{ACT_Config.nheads}/headdim-{ACT_Config.head_dim}"
    )
