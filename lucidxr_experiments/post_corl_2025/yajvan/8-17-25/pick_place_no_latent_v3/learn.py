...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.resnet_layer = [2,3,4]
        ACT_Config.norm_layer = ["FrozenBatchNorm2d","BatchNorm2d", "LayerNorm", "GroupNorm"]
        ACT_Config.backbone = ["resnet18", "resnet18_untrained"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/backbone-{ACT_Config.backbone}/norm-{ACT_Config.norm_layer}/resnet_layer-{ACT_Config.resnet_layer}/color_jitter-{ACT_Config.color_jitter}"
    )
