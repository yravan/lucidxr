...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.norm_layer = ["FrozenBatchNorm2d","BatchNorm2d"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/dec_layers-{ACT_Config.dec_layers}/backbone-{ACT_Config.backbone}/norm-{ACT_Config.norm_layer}/resnet_layer-{ACT_Config.resnet_layer}/color_jitter-{ACT_Config.color_jitter}"
    )
