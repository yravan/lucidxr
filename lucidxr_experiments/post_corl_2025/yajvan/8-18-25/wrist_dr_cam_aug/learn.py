...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'low-res' if ACT_Config.downsample_images > 1 else 'high-res'}dec_layers-{ACT_Config.dec_layers}/resnet_layer-{ACT_Config.resnet_layer}/color_jitter-{ACT_Config.color_jitter}"
    )
