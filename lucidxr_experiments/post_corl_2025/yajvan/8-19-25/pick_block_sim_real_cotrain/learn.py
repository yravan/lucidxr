...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.image_keys = [["right/rgb", "wrist/rgb", "left/rgb"],
                                  ["right/rgb", "wrist/rgb", "left/rgb"]]
    TrainArgs.real_robot = [False, True]
    TrainArgs.num_steps = 30000
    with sweep.product:
        TrainArgs.data_fractions = [[1.0, 1.0], [1.0, 0.5], [1.0, 0.0]]
        ACT_Config.lr_backbone = [2e-4]
        ACT_Config.color_jitter = [True, False]
        ACT_Config.resnet_layer = [3,4]
        ACT_Config.backbone = ["resnet18"]
        ACT_Config.chunk_size = [25]
        with sweep.zip:
            ACT_Config.dec_layers = [1, 4]
            ACT_Config.skip_encoder = [False, True]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/real-data-fraction-{TrainArgs.data_fractions[1]}/wrist-only-{not len(ACT_Config.image_keys) > 1}/resnet_layer-{ACT_Config.resnet_layer}/color_jitter-{ACT_Config.color_jitter}"
    )
