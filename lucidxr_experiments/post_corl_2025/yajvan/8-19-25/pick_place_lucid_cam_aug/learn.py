...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.downsample_images = [2]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/low-res/{'wrist-frame' if ACT_Config.delta_space else 'base-frame'}/chunk-size-{ACT_Config.chunk_size}/dec_layers-{ACT_Config.dec_layers}/nheads-{ACT_Config.nheads}/lr_backbone-{ACT_Config.lr_backbone}"
    )
