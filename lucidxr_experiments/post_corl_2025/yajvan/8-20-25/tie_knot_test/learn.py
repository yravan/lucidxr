...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'wrist-only' if len(ACT_Config.image_keys) == 1 else 'all-three'}/{'wrist-frame' if ACT_Config.delta_space else 'base-frame'}/resnet_layer-{ACT_Config.resnet_layer}/lr_backbone-{ACT_Config.lr_backbone}"
    )
