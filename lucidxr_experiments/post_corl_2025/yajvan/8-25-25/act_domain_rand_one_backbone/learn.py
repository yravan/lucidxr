...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.separate_backbone = False
    with sweep.product:
        ACT_Config.delta_space = [False]
        ACT_Config.downsample_images = [1]
        ACT_Config.enc_layers = [4]
        ACT_Config.dec_layers = [1,2,4,7]
        ACT_Config.kl_weight = [10, 100]
        ACT_Config.nheads = [8]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/dec_layers-{ACT_Config.dec_layers}/kl-{ACT_Config.kl_weight}"
    )
