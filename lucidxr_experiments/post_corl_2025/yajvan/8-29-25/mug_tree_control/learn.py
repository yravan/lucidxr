...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.chunk_size = 48
    TrainArgs.vis_dim = 512

    TrainArgs.n_steps = 100000
    TrainArgs.seed = 100
    TrainArgs.step_checkpoint_interval = 10000
    TrainArgs.backbone = 'cnn'
    with sweep.product:
        ...
        TrainArgs.image_keys = [
            ["wrist/rgb"], ["right/rgb", "wrist/rgb", "left/rgb"]
        ]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'wrist-only' if len(TrainArgs.image_keys) == 1 else 'all-three'}",
    )
