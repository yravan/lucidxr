...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            TrainArgs.dataset_prefix = [demo_prefixes[i:i+3] for i in range(0, len(demo_prefixes) - 3)]
            ACT_Config.task_name = [i for i in range(len(demo_prefixes) - 3)]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{ACT_Config.task_name}"
    )
