...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            ACT_Config.image_keys = [["right/lucid", "wrist/lucid", "left/lucid"], ["wrist/lucid"]]
            TrainArgs.dataset_prefix = [demo_prefixes[:2], demo_prefixes[:1]]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image-keys-{'-'.join([k.split('/')[0] for k in ACT_Config.image_keys])}"
    )
...
with Sweep(RUN, Unroll, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            Unroll.seed = [*range(15)]
            Unroll.render = [True for _ in range(5)] + [False for _ in range(10)]  # only render 10% of the runs
