...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    TrainArgs.dataset_prefix = demo_prefixes[4:6]
    ACT_Config.kl_weight = 10
    with sweep.product:
        ACT_Config.chunk_size = [100, 150, 200]
        ACT_Config.image_keys = [["wrist/rgb"]]
        with sweep.zip:
            TrainArgs.eval_interval = [16000, 2000]
            TrainArgs.checkpoint_interval = [16000, 2000]
            TrainArgs.num_steps = [240000, 30000]
            ACT_Config.dim_feedforward = [3200, 256]
            ACT_Config.head_dim = [512, 128]
            ACT_Config.lr = [1e-5, 1e-4]
            ACT_Config.lr_backbone = [1e-6, 1e-5]
            TrainArgs.batch_size_train = [8, 64]
            TrainArgs.batch_size_eval = [8, 64]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunk_size-{ACT_Config.chunk_size}/bs-{TrainArgs.batch_size_train}",
    )
