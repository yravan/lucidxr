...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ACT_Config.chunk_size = [25, 100]
        with sweep.zip:
            TrainArgs.num_steps = [50000, 100000]
            ACT_Config.image_keys = [["right/domain_rand", "wrist/domain_rand", "left/domain_rand"], ["wrist/domain_rand"]]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/domain_rand','')}/chunksize-{ACT_Config.chunk_size}"
    )
