...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/use_aux_loss-{TrainArgs.use_aux_loss}/datasets-{len(TrainArgs.dataset_prefix)}/aug_camera_randomization-{TrainArgs.aug_camera_randomization}/{TrainArgs.seed}"
    )
    checkpoints[
