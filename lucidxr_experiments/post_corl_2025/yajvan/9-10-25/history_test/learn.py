...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.step_checkpoint_interval = 30000
    DiffusionPolicyArgs.channels = [128, 256, 512, 1024]
    with sweep.product:
        # TrainArgs.use_aux_loss = [True, False]
        DiffusionPolicyArgs.history_length = [2, 4, 8]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/big-unet/{'rgb' if DiffusionPolicyArgs.image_keys[0] == 'wrist/rgb' else 'lucid'}/history-{DiffusionPolicyArgs.history_length}"
    )
