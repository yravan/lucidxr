...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/big-unet/{'all-three' if len(DiffusionPolicyArgs.image_keys) > 1 else 'wrist-only'}/aug_camera_randomization-{TrainArgs.aug_camera_randomization}/{TrainArgs.seed}"
    )
