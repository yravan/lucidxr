...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.n_steps = 500000
    TrainArgs.step_checkpoint_interval = 20000
    TrainArgs.eval_ema = True
    with sweep.product:
        with sweep.zip:
            TrainArgs.weight_decay =[0, 1e-4]
            TrainArgs.lr_start = [1e-3,1e-4]
            DiffusionPolicyArgs.score_net = ['unet',"causal_transformer"]
        TrainArgs.seed = [100]
        with sweep.zip:
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            ...
            Unroll.image_keys = [
                ["front/rgb", "wrist/rgb", "front_r/rgb"],
            ]
