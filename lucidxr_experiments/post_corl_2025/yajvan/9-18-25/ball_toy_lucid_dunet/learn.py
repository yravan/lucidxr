...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys)==3 else 'wrist-only'}/{'camera-aug' if TrainArgs.aug_camera_randomization else 'no-camera-aug'}/{TrainArgs.seed}"
    )
