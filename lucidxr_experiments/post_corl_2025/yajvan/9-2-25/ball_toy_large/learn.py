...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/datasets-{len(TrainArgs.dataset_prefix)}/{'wrist-only' if len(DiffusionPolicyArgs.image_keys) == 1 else 'all-three'}/{DiffusionPolicyArgs.backbone}"
    )
    checkpoints[
