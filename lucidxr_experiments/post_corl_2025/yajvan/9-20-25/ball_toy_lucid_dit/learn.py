...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    DiffusionPolicyArgs.transformer_pos_embed = "learned"
    with sweep.product:
        TrainArgs.aug_camera_randomization = [True]
        DiffusionPolicyArgs.transformer_vision_input = ["pixels", "features"]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/learned-pos-embed/{DiffusionPolicyArgs.transformer_vision_input}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys)==3 else 'wrist-only'}/{'camera-aug' if TrainArgs.aug_camera_randomization else 'no-camera-aug'}/{TrainArgs.seed}"
    )
