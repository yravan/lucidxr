...
demo_prefixes = [
    ...
    ["/lucidxr/lucidxr/datasets/lucidxr/corl-2025/pick_place_robot_room/2025/08/07/15.58.28",
     ...
     "/lucidxr/lucidxr/datasets/lucidxr/corl-2025/pick_place_robot_room/2025/08/07/15.58.28_0"], # Pick Place
]
steps = [
    200000,
    300000,
...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.dataset_prefix = demo_prefixes[6]
    TrainArgs.n_steps = steps[6]
    TrainArgs.step_checkpoint_interval = TrainArgs.n_steps//5
    TrainArgs.eval_ema = True

    TrainArgs.weight_decay = 1e-4
    TrainArgs.lr_start = 8e-5
    TrainArgs.lr_end = 5e-6
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = [
                "PickPlaceRobotRoom-single_random-gsplat-cic_11th_stationary",
                "PickPlaceRobotRoom-single_random-gsplat-mit_medical_wood_chair",
                "PickPlaceRobotRoom-single_random-gsplat-cic_bench",
            ]
            Unroll.image_keys = [
                ["stereo_far_left/splat_rgb", "wrist/splat_rgb", "stereo_far_right/splat_rgb"],
                ["stereo_far_left/splat_rgb", "wrist/splat_rgb", "stereo_far_right/splat_rgb"],
                ["stereo_far_left/splat_rgb", "wrist/splat_rgb", "stereo_far_right/splat_rgb"],
            ]
        with sweep.zip:
