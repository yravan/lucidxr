...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    with sweep.product:
        DiffusionPolicyArgs.score_net = ["ddpm_transformer"]
        TrainArgs.seed = [100,]
        ...
        with sweep.zip:
            TrainArgs.color_jitter = [True, False]
            TrainArgs.gaussian_blur = [True, False]
            TrainArgs.hue_jitter = [True, False]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys)==3 else 'wrist-only'}/{'color-jitter'if TrainArgs.color_jitter else 'no-color-jitter'}/{TrainArgs.seed}"
    )
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = [
                "MugTree-mug_rand-gsplat-cic_4th_nook",
                "MugTree-mug_rand-gsplat-cic_kitchen_4th_day",
                "MugTree-mug_rand-gsplat-google_building_table",
            ]
            Unroll.image_keys = [
                ["stereo_far_left/splat_rgb-cic_4th_nook", "wrist/splat_rgb-cic_4th_nook", "stereo_far_right/splat_rgb-cic_4th_nook"],
                ["stereo_far_left/splat_rgb-cic_kitchen_4th_day", "wrist/splat_rgb-cic_kitchen_4th_day", "stereo_far_right/splat_rgb-cic_kitchen_4th_day"],
