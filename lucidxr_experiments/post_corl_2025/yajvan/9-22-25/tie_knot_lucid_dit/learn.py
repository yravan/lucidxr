...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.dataset_prefix = demo_prefixes[5]
    TrainArgs.n_steps = steps[5]
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = [
                "TieKnot-random-gsplat-mit_medical_wood_chair",
                "TieKnot-random-gsplat-cic_bench",
                "TieKnot-random-gsplat-google_building_table",
            ]
            Unroll.image_keys = [
