...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    with sweep.product:
        ...
        DiffusionPolicyArgs.chunk_size = [48, 24]
        with sweep.zip:
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunk-size-{DiffusionPolicyArgs.chunk_size}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys)==3 else 'wrist-only'}/{TrainArgs.seed}"
    )
