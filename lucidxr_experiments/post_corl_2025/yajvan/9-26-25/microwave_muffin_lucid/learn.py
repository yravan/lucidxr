...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    TrainArgs.dataset_prefix = demo_prefixes[1]
    TrainArgs.n_steps = steps[1]
    ...
    with sweep.product:
        ...
        DiffusionPolicyArgs.image_keys = [
            # ["wrist/lucid", "front/lucid", "front_r/lucid"],
            ["wrist/lucid"]
        ]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunksize-{DiffusionPolicyArgs.chunk_size}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys)==3 else 'wrist-only'}/{TrainArgs.seed}"
    )
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = [
                "MicrowaveMuffin-Random-gsplat-cic_4th_nook",
                "MicrowaveMuffin-Random-gsplat-cic_11th_kitchen",
                "MicrowaveMuffin-Random-gsplat-mit_medical_wood_chair",
            ]
            Unroll.image_keys = [
                ["front/splat_rgb-cic_4th_nook", "wrist/splat_rgb-cic_4th_nook", "front_r/splat_rgb-cic_4th_nook"],
                ["front/splat_rgb-cic_11th_kitchen", "wrist/splat_rgb-cic_11th_kitchen", "front_r/splat_rgb-cic_11th_kitchen"],
                ["front/splat_rgb-mit_medical_wood_chair", "wrist/splat_rgb-mit_medical_wood_chair", "front_r/splat_rgb-mit_medical_wood_chair"],
            ]
