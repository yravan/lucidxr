...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            TrainArgs.seed = [300,200]
            TrainArgs.load_checkpoint = [
                "/lucidxr/lucidxr/post_corl_2025/yajvan/9-26-25/microwave_muffin/learn/2025/09/25/21-50-02/chunk-size-24/unet/wrist-only/300/checkpoints/policy_latest.pt",
                "/lucidxr/lucidxr/post_corl_2025/yajvan/9-26-25/microwave_muffin/learn/2025/09/25/21-50-02/chunk-size-24/unet/wrist-only/200/checkpoints/policy_latest.pt",
            ]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/chunk-size-{DiffusionPolicyArgs.chunk_size}/{DiffusionPolicyArgs.score_net}/{'all-three' if len(DiffusionPolicyArgs.image_keys) == 3 else 'wrist-only'}/{TrainArgs.seed}",
    )
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = [
                "MicrowaveMuffin-Random-v1",
            ]
            Unroll.image_keys = [
                ["front/rgb", "front_r/rgb", "wrist/rgb"],
            ]
