...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/dataset-{len(TrainArgs.dataset_prefix)}/{TrainArgs.seed}"
    )
...
with Sweep(RUN, Unroll, TrainArgs) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            ...
            Unroll.render = [True for _ in range(20)] + [False for _ in range(10)]  # only render 10% of the runs
