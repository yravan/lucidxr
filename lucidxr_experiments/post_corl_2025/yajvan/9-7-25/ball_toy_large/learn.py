...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            ...
            DiffusionPolicyArgs.action_len = [12, 24]
        TrainArgs.seed = [100]
        with sweep.zip:
        ...
        DiffusionPolicyArgs.embed_dim = [128, 512]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/datasets-{len(TrainArgs.dataset_prefix)}/embed_dim-{DiffusionPolicyArgs.embed_dim}/chunk_size-{DiffusionPolicyArgs.chunk_size}/{TrainArgs.seed}"
    )
    checkpoints[
