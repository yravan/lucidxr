...
with Sweep(RUN, TrainArgs, DiffusionPolicyArgs) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            TrainArgs.n_steps = [200000, 400000]
            TrainArgs.step_checkpoint_interval = [20000, 40000]
            TrainArgs.aug_camera_randomization = [False, True]
...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/{'lucid' if 'lucid' in DiffusionPolicyArgs.image_keys[0] else 'rgb'}/backbone-{DiffusionPolicyArgs.backbone}/dataset-{len(TrainArgs.dataset_prefix)}/{TrainArgs.seed}"
    )
