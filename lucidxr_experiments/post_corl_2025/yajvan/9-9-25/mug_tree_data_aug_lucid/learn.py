...
def tail(RUN, TrainArgs, DiffusionPolicyArgs):
    RUN.prefix, RUN.job_name, _ = RUN(
        script_path=__file__, job_name=f"{time:%Y/%m/%d/%H-%M-%S}/backbone-{DiffusionPolicyArgs.backbone}/dataset-{len(TrainArgs.dataset_prefix)}/{TrainArgs.seed}"
    )
