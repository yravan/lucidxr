...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        with sweep.zip:
            ACT_Config.dim_feedforward = [3200, 3200, 256, 256]
            ACT_Config.head_dim = [512, 128, 512, 128]
        TrainArgs.seed = [0]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/dim_feedforward-{ACT_Config.dim_feedforward}/head_dim-{ACT_Config.head_dim}",
    )
