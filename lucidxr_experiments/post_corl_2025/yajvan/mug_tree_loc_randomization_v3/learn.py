...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.lr = 1e-4
    with sweep.product:
        ACT_Config.chunk_size = [25, 50, 75, 100]
        ACT_Config.image_keys = [["right/rgb", "wrist/rgb", "left/rgb"], ["right/rgb", "wrist/rgb"], ["wrist/rgb"]]
        TrainArgs.seed = [0]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb','')}/chunksize-{ACT_Config.chunk_size}/kl-{ACT_Config.kl_weight}/lr-{ACT_Config.lr}/{TrainArgs.seed}",
    )
...
with Sweep(RUN, Unroll, ACT_Config) as sweep:
    ...
    with sweep.product:
        with sweep.zip:
            Unroll.env_name = ["MugTree-mug_rand-v1", "MugTree-fixed-v1"]
