...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ACT_Config.kl_weight = [1e-4, 1e-3, 1e-2, 1e-1, 1.0, 10]
        TrainArgs.seed = [0]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb','')}/chunksize-{ACT_Config.chunk_size}/lr-{ACT_Config.lr}/kl-{ACT_Config.kl_weight}/{TrainArgs.seed}",
    )
