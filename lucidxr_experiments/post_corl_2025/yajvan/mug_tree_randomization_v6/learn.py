...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ...
        ACT_Config.kl_weight = [0.01, 0.1, 1, 10]
        TrainArgs.seed = [0]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"{time:%Y/%m/%d/%H-%M-%S}/image_keys-{'-'.join(ACT_Config.image_keys).replace('/rgb','')}/chunksize-{ACT_Config.chunk_size}/kl-{ACT_Config.kl_weight}/{TrainArgs.seed}",
    )
