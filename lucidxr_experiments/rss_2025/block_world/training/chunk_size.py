...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ACT_Config.kl_weight = [50]
        ACT_Config.lr = [1e-5]
        ACT_Config.chunk_size = [5, 10, 20, 50, 100, 200]
        TrainArgs.seed = [20]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"pick_block_chunk_size/{time}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
