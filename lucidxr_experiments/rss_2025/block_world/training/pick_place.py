...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    with sweep.product:
        ACT_Config.kl_weight = [10, 20, 50, 100]
        ACT_Config.lr = [1e-5, 5e-5, 1e-4, 5e-4]
        TrainArgs.seed = [10, 20, 30]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"pick_place/{time}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
