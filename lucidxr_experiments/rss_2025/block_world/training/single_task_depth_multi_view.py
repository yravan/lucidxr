...
with Sweep(RUN, TrainArgs, ACT_Config) as sweep:
    ...
    ACT_Config.image_keys = ["front/depth", "left/depth", "wrist/depth"]
...
def tail(RUN, TrainArgs, ACT_Config):
    RUN.prefix, RUN.job_name, _ = RUN(
        ...
        job_name=f"single_task_depth_multi_view/{time}/lr-{ACT_Config.lr:0.0e}/kl-{ACT_Config.kl_weight:0.0e}/{TrainArgs.seed}",
    )
