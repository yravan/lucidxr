...
def make_schema(robot="panda", **options):
    ...
    mug_tree = MjSDF(
        pos=[x1, y1, 0.95], assets="mug_tree", _attributes={"name": "mug-tree", "quat": "0.707 0.707 0 0"}, scale=".17 .17 .17"
    )
    ...
    mug = VuerMug(pos=[x2, y2, 0.85], quat=[0, 0, 1, 0])
...
def register():
    ...
    add_env(
        env_id="Mug_tree_task-v1",
        entrypoint=make_env,
        kwargs=dict(
            xml_path="mug_tree_task.mjcf.xml",
            workdir=Path(__file__).parent,
            mode="render",
        ),
    )
    add_env(
        env_id="Mug_tree_task-lucid-v1",
        entrypoint=make_env,
        kwargs=dict(
            xml_path="mug_tree_task.mjcf.xml",
            workdir=Path(__file__).parent,
