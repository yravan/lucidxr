...
@contextmanager
def invisibility(physics, geom_ids: list):
    try:
        # Check if the geom exists
        original_rgbas = []
        for geom_id in geom_ids:
            original_rgba = physics.model.geom(geom_id).rgba.copy()
            # Set RGBA to fully transparent to hide the geom
            physics.model.geom(geom_id).rgba = [0, 0, 0, 0]
    ...
    try:
        ...
        if geom_exists:
            ...
            for geom_id, original_rgba in zip(geom_ids, original_rgbas):
                physics.model.geom(geom_id).rgba = original_rgba

def turn_off_site_visibility(physics):
    for id in range(physics.model.nsite):
        # Set RGBA to fully transparent to hide the geom
        physics.model.site(id).rgba[-1] = 0
